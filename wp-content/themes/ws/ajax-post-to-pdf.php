...
//  Check if image was properly decoded
    if ($decodedImg !== false) {
        ...
        if (file_put_contents( $TEMPIMGLOC, $decodedImg)  ) {

            define('SECTION',chr(167));
            $pdf = new PDF();
            $title = iconv('UTF-8', 'windows-1252',$_POST['product_name']);
            $price = iconv('UTF-8', 'windows-1252', '£'.$_POST['price']);
            $pdf->AddPage();
            //  $pdf->SetTitle('Waterstones Gift Cards');
            //  $pdf->Cell(0,10,'Product image',0,1);
            //$pdf->Cell( 40, 40, $pdf->Image($_POST['image'], $pdf->GetX(), $pdf->GetY(), 33.78), 0, 0, 'L', false );
            //  $pdf->Cell(0,10,$pdf->Image($_POST['image'], $pdf->GetX(), $pdf->GetY(), 33.78), 0, 0, 'L',0,1);
            $pdf->Image($_POST['image'],0,0,210);



            $pdf->Cell(0,150,$title,0,0,'C');
            $pdf->SetX(10);
            $pdf->Cell( 0, 170, $price, 0, 0, 'C' );
            $pdf->SetX(10);
            $pdf->SetFont('Arial','B',12);
            $pdf->Cell( 0, 265, $_POST['code'], 0, 0, 'C' );
            $pdf->SetX(10);

            $pdf->Image($TEMPIMGLOC , 55, 125, 100);
            // $pdf->MultiCell(0,'330',$_POST['code'],0);
            $filename = dirname(__FILE__, 3) . '/uploads/pdfs/'.$_POST['product_id'].'.pdf';
